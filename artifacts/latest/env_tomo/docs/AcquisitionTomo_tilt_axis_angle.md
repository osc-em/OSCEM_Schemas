
# Slot: tilt_axis_angle

The tilt axis angle of a tomography series

URI: [https://w3id.org/oscem-schemas/latest/env_tomo/AcquisitionTomo_tilt_axis_angle](https://w3id.org/oscem-schemas/latest/env_tomo/AcquisitionTomo_tilt_axis_angle)


## Domain and Range

[AcquisitionTomo](AcquisitionTomo.md) &#8594;  <sub>1..1</sub> [Float](types/Float.md)

## Parents

 *  is_a: [tilt_axis_angle](tilt_axis_angle.md)

## Children


## Used by

 * [AcquisitionCelTomo](AcquisitionCelTomo.md)
 * [AcquisitionEnvTomo](AcquisitionEnvTomo.md)
 * [AcquisitionSubTomo](AcquisitionSubTomo.md)
 * [AcquisitionTomo](AcquisitionTomo.md)
